...
export default {
  ...
  mounted() {
    this.categoryMenu.forEach((elem) => {
      if (elem.name === this.prevArticleContent.study_ongoing) {
        this.selectedCategoryMenuIdx = elem.id;
      }
    });

    this.titleInput = this.prevArticleContent.study_title;

    this.selectedParticpantNum = this.prevArticleContent.study_participant;

    this.prevArticleContent.study_skill.split(",").forEach((pName) => {
    ...
    this.editor = new Editor({
      ...
      content: this.prevArticleContent.study_markup,
    });
  },
  methods: {
    ...
    async saveArticle() {
      ...
      try {
        ...
        paramNewArticleData["participant"] = this.selectedParticpantNum;
